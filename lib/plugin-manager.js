...
exports.PluginManager = class PluginManager {
  ...
  constructor (registry, cwd) {
    ...
    let defaultLoader = require('./plugin-loader')
    registry.register(EP.PLUGIN_LOADER, defaultLoader)
    
    this._loaders = registry.getExtensionPoint(EP.PLUGIN_LOADER)

    let ep = registry.getExtensionPoint(EP.PLUGIN_ID)
  ...
  load (pathOrId) {
    ...
    return loader.loadJsonFromModule(base, 'package.json')
      .then((json) => {
        packageJson = json
        plugin = new Plugin(base, packageJson.name)
  ...
  _loadPluginJavascript(plugin, path) {
    return Plugin.prototype._loadPluginJavascript.call(plugin, path)
  }

  _reifyExtension(plugin, path) {
    plugin.prototype = new Plugin()
    return Plugin.prototype._reifyExtension.call(plugin, path)
  }
