...
exports.PluginManager = class PluginManager {
  ...
  constructor (registry, cwd) {
    ...
    let restart = function () {
      let plugin = pluginManager._pluginsByName[this.name]
      return pluginManager.unload(plugin.location)
        .then(() => {
          return pluginManager.load(plugin.location)
        })
    }
    restart._id = 'restartSelf'
    restart.description = 'Restart the plugin. You cannot use this to restart other plugins.'
    registry.register(EP.SINGLETONS, restart)
