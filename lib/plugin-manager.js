...
exports.PluginManager = class PluginManager {
  ...
  _load (ids) {
    ...
    let loadManifests = ids.map((pathOrId) => {
      return this._loadSinglePlugin(pathOrId)
        .catch((ex) => {
          console.error(ex.stack)
        })
    })

    Promise.all(loadManifests)
      .then((plugins) => {
  ...
  load (pathOrId) {
    ...
    plugin = new Plugin(base)
    return plugin.intializeFromDisk(loader)
      .then(() => {
        let manifest = plugin._manifest
        let isPlugin = false
        if (manifest) {
          this.unpackManifest(plugin, manifest)
  ...
  unpackManifest (plugin, manifest) {
    let registry = this._registry
  ...
  _loadPluginJavascript (extPath) {
    ...
    if (!moduleExports) {
      ...
      try {
        ...
        throw new Error('Error loading ' + modulePath + ': ' + e.message)
      }
    }
